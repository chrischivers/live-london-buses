...
import scalacache.{ScalaCache, put, _}

class SourceLineHandler(definitions: Definitions)(implicit scalaCache: ScalaCache[NoSerialization], executionContext: ExecutionContext) extends StrictLogging {
...
//    def updateFirstStop: OptionT[Future, Unit] = {
//      logger.debug("Updating first stop in cache")
//      for {
//        _ <- OptionT.liftF(put(sourceLine.vehicleID, sourceLine.route, sourceLine.direction, indexOfStop)(sourceLine.arrival_TimeStamp))
//        _ <- OptionT.liftF(put(sourceLine.vehicleID, sourceLine.route, sourceLine.direction)(indexOfStop))
//      } yield ()
//    }
    ...
    OptionT.liftF(get[LastIndexPersisted, NoSerialization](sourceLine.vehicleID, sourceLine.route, sourceLine.direction)).flatMap {
      ...
      //      case _ if indexOfStop == 0 => updateFirstStop
      case _ => updateStop
    }
