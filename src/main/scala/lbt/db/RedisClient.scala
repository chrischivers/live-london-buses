...
class RedisClient(redisConfig: RedisConfig)(implicit executionContext: ExecutionContext) extends StrictLogging {
  ...
  def getStopToStopTimes(route: BusRoute, fromStopSeq: Int, toStopSeq: Int): Option[List[Int]] = {
    val key = getKey(route, fromStopSeq, toStopSeq)
