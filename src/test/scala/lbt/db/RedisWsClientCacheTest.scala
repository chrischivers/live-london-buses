...
class RedisWsClientCacheTest extends fixture.FunSuite with SharedTestFeatures with ScalaFutures with OptionValues with EitherValues {
  ...
  test("Positioning Data where next arrival time is in future are available in InProgress cache") { f =>
    ...
    f.redisWSClientCache.storeVehicleActivityInProgress(busPosData1).futureValue
    f.redisWSClientCache.storeVehicleActivityInProgress(busPosData2).futureValue
